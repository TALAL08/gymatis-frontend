...
export function ToggleAccountStatusDialog({ open, onOpenChange, account, onSuccess }: DeactivateAccountDialogProps) {
  const [loading, setLoading] = useState(false);

  const handleDeactivate = async () => {
    setLoading(true);
    try {
      await AccountService.toggleStatus(account.id,!account.isActive);
